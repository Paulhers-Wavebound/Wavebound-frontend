...
import { fmtNum, StatChip, SectionCard, countryName, countryFlag } from "./shared";
...
export default function GrowthTab({
  ...
  // Audience footprint
  const platforms = [
    { name: "TikTok", followers: data.tiktokFollowers, growth: data.tiktokGrowth7d, color: "#FF004F" },
    { name: "Instagram", followers: data.instagramFollowers, growth: data.instagramGrowth7d, color: "#E1306C" },
    { name: "YouTube", followers: data.youtubeSubscribers, growth: data.youtubeGrowth7d, color: "#FF0000" },
    { name: "Spotify", followers: data.spotifyFollowers, growth: null, color: "#1DB954" },
  ...
  const urgencyGroups = useMemo(() => {
    const groups: Record<string, typeof markets> = { act_now: [], plan: [], monitor: [] };
    ...
    <div className="space-y-4">
        ...
        <SectionCard title="Audience Footprint">
          {data.totalSocialReach != null && (
            <div className="mb-5">
              ...
              <span className="text-[11px] text-white/35 ml-2 uppercase tracking-wider">Total Reach</span>
            </div>
          )}
          ...
          <div className="space-y-2.5">
            {platforms.map((p) => (
              <div key={p.name} className="flex items-center gap-3">
                <span className="text-[12px] font-medium text-white/55 w-[72px] shrink-0">{p.name}</span>
                <div className="flex-1 h-2.5 rounded-full bg-white/[0.04] overflow-hidden">
                  <div className="h-full rounded-full transition-all duration-500"
                    style={{ width: `${((p.followers || 0) / maxFollowers) * 100}%`, background: p.color, minWidth: 4 }} />
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <SectionCard title="Touring Signal">
            <div className="flex items-center gap-3 mb-4">
              {ts.touringStatus && (
                <span
                  ...
                  style={{
                    ...
                    background: TOURING_STATUS[ts.touringStatus]?.bg ?? "rgba(142,142,147,0.12)",
                  }}
                >
              ...
              <div className="flex items-baseline gap-2">
                ...
                <span className="text-[11px] text-white/35 uppercase tracking-wider">Upcoming Events</span>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <StatChip label="Ticketmaster" value={String(ts.ticketmasterUpcoming)} />
              <StatChip label="New This Week" value={String(ts.newEventsAnnounced7d)}
        ...
        {rosterScores.length >= 2 && currentRank > 0 && (
          <SectionCard title="Roster Rank">
            <div className="flex items-baseline gap-3 mb-3">
              <span
                className="text-[28px] font-bold font-mono tabular-nums leading-none"
                style={{ color: currentRank <= 3 ? "var(--accent)" : "rgba(255,255,255,0.87)" }}
              >
                #{currentRank}
              </span>
              <span className="text-[12px] text-white/35 font-mono">of {sorted.length}</span>
            ...
            {sorted.length >= 5 && (() => {
              ...
              for (const s of sorted) { buckets[Math.min(Math.floor(s.artist_score / 10), 9)]++; }
              const maxB = Math.max(...buckets, 1);
              const curB = artistScore != null ? Math.min(Math.floor(artistScore / 10), 9) : -1;
              return (
                <div className="flex items-end gap-1 h-[24px] mb-2">
                  {buckets.map((c, i) => (
                    <div key={i} style={{ flex: 1, height: Math.max((c / maxB) * 20, 2), borderRadius: "2px 2px 0 0", background: i === curB ? "var(--accent)" : "rgba(255,255,255,0.06)" }} />
                  ))}
                </div>
              );
            })()}
            ...
            {artistScore != null && (() => {
              const avgScore = Math.round(sorted.reduce((s, e) => s + e.artist_score, 0) / sorted.length);
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          <span className="text-sm">{countryFlag(m.countryCode)}</span>
                          <span className="text-[13px] font-medium text-white/87">
                            {countryName(m.countryCode)}
                          </span>
                          <span className="text-[10px] text-white/25 font-mono">{m.countryCode}</span>
                        </div>
                        <span className="text-[14px] font-bold text-white/75 font-mono tabular-nums">
                          {m.opportunityScore}
                        </span>
                      </div>
                      <div className="flex flex-wrap gap-1.5">
                        ...
                        {m.windowConfidence && (
                          <span className="text-[9px] text-white/30 px-1.5 py-0.5 rounded bg-white/[0.03]">
                      ...
                      {m.entrySongName && (
                        <div className="text-[11px] text-white/40 mt-1.5">
                          Entry: <span className="text-white/55">"{m.entrySongName}"</span>
                        </div>
                      )}
                      ...
                      {m.platformToActivateFirst && (
                        <div className="text-[11px] text-white/30 mt-0.5">
                          Activate on: <span className="font-medium text-white/50">{m.platformToActivateFirst}</span>
                        </div>
                      )}
                    </div>
                  ))}
      ...
      {platforms.length === 0 && markets.length === 0 && !ts && (
        <div className="rounded-xl border border-white/[0.06] p-8 text-center" style={{ background: "#1C1C1E" }}>
          <p className="text-[14px] text-white/30">Growth data not yet available for this artist</p>
        </div>
      )}
    </div>
  );
