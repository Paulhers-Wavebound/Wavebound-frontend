...
import { CircleAlert, TriangleAlert, Info, CheckCircle, ChevronDown, ChevronLeft, ChevronRight } from 'lucide-react';
...
const severityConfig: Record<string, { icon: typeof CircleAlert; border: string; iconClass: string }> = {
  critical: { icon: CircleAlert, border: 'border-l-red-500', iconClass: 'text-red-500' },
  warning:  { icon: TriangleAlert, border: 'border-l-amber-500', iconClass: 'text-amber-500' },
  info:     { icon: Info, border: 'border-l-blue-500', iconClass: 'text-blue-500' },
};

export default function RiskAlertsPanel({ metrics }: { metrics: RosterMetric[] }) {
  ...
  const alerts = useMemo<FlatAlert[]>(() => {
    ...
    flat.sort((a, b) => (severityOrder[a.severity] ?? 9) - (severityOrder[b.severity] ?? 9));
    return flat;
  }, [metrics]);
        ...
        <div className="relative">
          ...
          {showLeftFade && (
            <>
              ...
              <button onClick={() => scrollBy('left')} className="absolute left-0 top-1/2 -translate-y-1/2 z-20 p-1 rounded-full bg-card border border-border text-muted-foreground hover:text-foreground">
                <ChevronLeft size={14} />
              </button>
            </>
          )}
          {showRightFade && (
            <>
              ...
              <button onClick={() => scrollBy('right')} className="absolute right-0 top-1/2 -translate-y-1/2 z-20 p-1 rounded-full bg-card border border-border text-muted-foreground hover:text-foreground">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-muted-foreground">@{alert.artist_handle}</p>
                    </div>
                    <Button
                      variant="ghost"
                      size="sm"
                      className="shrink-0 text-xs h-7 px-2"
                      onClick={() => navigate(`/label/artists/${alert.artist_handle}`)}
                    >
                      View
                    </Button>
                  </div>
