...
function parseVerdict(verdict: string): { label: string; description: string } {
  ...
  const keywords = ['SCALE', 'STOP / FIX', 'STOP', 'MAINTAIN AT REDUCED VOLUME', 'MAINTAIN', 'REDUCE', 'TEST MORE', 'TEST', 'FIX'];
  for (const kw of keywords) {
    if (upper.startsWith(kw)) {
      const rest = verdict.trim().slice(kw.length).replace(/^[\s.,:;-]+/, '').trim();
  ...
  if (sentenceEnd > 0 && sentenceEnd < 30) {
    return { label: verdict.slice(0, sentenceEnd).trim().toUpperCase(), description: verdict.slice(sentenceEnd + 1).trim() };
  }
...
function verdictBadgeClass(label: string): string {
  ...
  if (upper === 'SCALE') return 'bg-green-500/15 text-green-500 border-green-500/30';
  if (upper === 'STOP' || upper === 'STOP / FIX' || upper === 'FIX') return 'bg-red-500/15 text-red-500 border-red-500/30';
  if (upper === 'REDUCE' || upper.startsWith('MAINTAIN') || upper.startsWith('TEST')) return 'bg-amber-500/15 text-amber-500 border-amber-500/30';
    ...
    <Card className="border-border/50 bg-card overflow-hidden rounded-xl">
      <Collapsible open={open} onOpenChange={setOpen}>
        <CollapsibleTrigger asChild>
          <button type="button" className="w-full flex items-center justify-between px-4 py-3 hover:bg-accent/20 transition-colors cursor-pointer">
...
function ContentXRay({ brandDocument, contentAnalysis }: { brandDocument: any; contentAnalysis: any }) {
  ...
  const sorted = formats?.length
    ? [...formats].sort((a: any, b: any) =>
        (safeNum(b.avg_performance_ratio ?? b.avg_view_multiplier) ?? 0) - (safeNum(a.avg_performance_ratio ?? a.avg_view_multiplier) ?? 0)
      )
    : [];
    ...
    <Section title="Content X-Ray">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {sorted.map((f: any, i: number) => {
                ...
                const perf = safeNum(f.avg_performance_ratio ?? f.avg_view_multiplier);
                return (
                  <tr key={i} className="border-b border-border/30 last:border-0">
                    <td className="py-2.5 pr-4 font-medium text-foreground whitespace-nowrap">{f.format_name ?? f.format ?? '—'}</td>
                    <td className="py-2.5 pr-4 text-right text-muted-foreground tabular-nums">{f.video_count ?? f.count ?? '—'}</td>
                    <td className={cn("py-2.5 pr-4 text-right tabular-nums font-medium", perf != null ? prColor(perf) : 'text-muted-foreground')}>
                    ...
                    <td className="py-2.5 text-xs text-muted-foreground/80 max-w-[200px] truncate">{description || '—'}</td>
                  </tr>
                );
              })}
            </tbody>
      ...
      {summary && (
        <div className="flex flex-wrap gap-4 mt-4 pt-4 border-t border-border/50 text-xs text-muted-foreground">
          {summary.videos_analyzed != null && <span>{summary.videos_analyzed} videos analyzed</span>}
          {summary.avg_hook_score != null && <span>Avg hook score: <b className="text-foreground">{summary.avg_hook_score}/10</b></span>}
          {summary.median_views != null && <span>Median views: <b className="text-foreground">{fmtNum(summary.median_views)}</b></span>}
        </div>
      )}
    </Section>
  );
...
function FormatCard({ name, verdictLabel, verdictDesc, count, perf }: {
  name: string;
  verdictLabel: string;
  verdictDesc: string;
  count: number | null;
  ...
  return (
    <div className="rounded-lg bg-muted/30 border border-border/40 p-3.5 space-y-1.5">
      ...
      <p className="text-xs text-muted-foreground">
        {count != null ? `${count} videos` : '—'} · {perf != null ? `Avg ${perf.toFixed(1)}x` : '—'}
      </p>
      {verdictDesc && (
        <>
          <p
            className={cn("text-xs text-muted-foreground/80 leading-relaxed cursor-pointer", !expanded && "line-clamp-1")}
...
function BestWorstPerformers({ contentAnalysis }: { contentAnalysis: any }) {
  ...
  const sorted = [...videos].sort((a: any, b: any) =>
    (b.performance_ratio ?? b.view_multiplier ?? 0) - (a.performance_ratio ?? a.view_multiplier ?? 0)
  );
    ...
    <Section title="Best & Worst Performers">
      {top3.length > 0 && (
        <div className="space-y-2">
          ...
          {top3.map((v: any, i: number) => <VideoRow key={`top-${i}`} video={v} type="top" />)}
        </div>
      )}
      {bottom3.length > 0 && (
        <div className="space-y-2 mt-4">
          ...
          {bottom3.map((v: any, i: number) => <VideoRow key={`bot-${i}`} video={v} type="bottom" />)}
        </div>
      )}
    </Section>
  );
...
function VideoRow({ video, type }: { video: any; type: 'top' | 'bottom' }) {
  ...
  const borderClass = type === 'top' ? 'border-l-green-500' : 'border-l-red-500';

  return (
    <div className={cn("flex items-center gap-3 p-2.5 rounded-lg border-l-2 bg-muted/30", borderClass)}>
      ...
      <div className="flex-1 min-w-0">
        <p className="text-xs text-foreground truncate">
          {video.caption ? (video.caption.length > 50 ? video.caption.slice(0, 50) + '…' : video.caption) : 'No caption'}
        </p>
      ...
      {pr != null && (
        <span className={cn("text-xs font-bold shrink-0 tabular-nums", prColor(pr))}>
          {pr.toFixed(1)}x
        </span>
      )}
    </div>
...
function PlatformComparison({ crossPlatform }: { crossPlatform: any }) {
  ...
  type Row = { metric: string; tk: string; ig: string; tkWin: boolean; igWin: boolean };
  const rows: Row[] = [];
  ...
  const winner = tkWins > igWins ? 'TikTok' : igWins > tkWins ? 'Instagram' : null;

  return (
    <Section title="TikTok vs Instagram">
      {winner && (
        <p className="text-xs text-muted-foreground mb-3">
          <span className="font-medium text-foreground">{winner}</span> outperforms on {Math.max(tkWins, igWins)} of {rows.length} metrics
        </p>
      )}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border/50">
              <th className="text-left py-2.5 px-3 text-xs font-medium text-muted-foreground">Metric</th>
              <th className="text-center py-2.5 px-3 text-xs font-medium text-muted-foreground">📱 TikTok</th>
              <th className="text-center py-2.5 px-3 text-xs font-medium text-muted-foreground">📷 Instagram</th>
            </tr>
          </thead>
          <tbody>
            {rows.map(r => (
              <tr key={r.metric} className="border-b border-border/30 last:border-0">
                <td className="py-2.5 px-3 text-xs text-muted-foreground">{r.metric}</td>
                <td className={cn("py-2.5 px-3 text-center text-xs", r.tkWin ? "text-green-500 font-semibold" : "text-muted-foreground")}>
                  {r.tk}
                </td>
                <td className={cn("py-2.5 px-3 text-center text-xs", r.igWin ? "text-green-500 font-semibold" : "text-muted-foreground")}>
                  {r.ig}
                </td>
              </tr>
            ))}
...
export function ArtistDeepSections({ data }: { data: DeepResearchData }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <ContentXRay brandDocument={brandDocument} contentAnalysis={contentAnalysis} />
