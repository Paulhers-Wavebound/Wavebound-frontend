...
import {
  Video as BaseVideo,
  PhotoCarousel as BasePhotoCarousel,
} from "@/types/content";
...
export const WorkspaceNotesEditor = ({
  compact = false,
  autoFocus = false,
}: WorkspaceNotesEditorProps) => {
  ...
  const [selectedPhoto, setSelectedPhoto] = useState<PhotoCarousel | null>(
    null,
  );
  ...
  const savedCursorPositionRef = useRef<{ start: number; end: number } | null>(
    null,
  );
  ...
  const loadWorkspace = async () => {
    try {
      ...
      if (data) {
        ...
        const contentToLoad =
          cachedContent !== null ? cachedContent : data.notes || "";
        ...
        setTimeout(() => {
          if (notesEditorRef.current) {
            ...
            setTimeout(() => {
              const badges =
                notesEditorRef.current?.querySelectorAll("[data-type]");
  ...
  const saveCursorPosition = () => {
    const selection = window.getSelection();
    if (!selection || selection.rangeCount === 0 || !notesEditorRef.current)
      return;
  ...
  const handleBadgeClick = async (event: MouseEvent) => {
    ...
    if (type === "video" && videoId) {
      ...
      if (reelData) {
        const thumbnailUrl =
          thumbnailData?.thumbnail_url || thumbnailData?.url || null;
  ...
  const handleEditorDragOver = (event: DragEvent) => {
    BadgeDragDrop.handleEditorDragOver(
      event,
  ...
  const handleEditorDragLeave = (event: DragEvent) => {
    BadgeDragDrop.handleEditorDragLeave(
      event,
      notesEditorRef.current,
      dropIndicatorRef,
    );
          ...
          mutation.addedNodes.forEach((node) => {
            if (node.nodeType === Node.ELEMENT_NODE) {
              ...
              if (
                ...
                element.querySelector?.("[data-type]")
              ) {
                shouldSetup = true;
              }
            }
  ...
  const currentWordCount = notesEditorRef.current
    ? countWords(notesEditorRef.current.innerText)
    : 0;
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex-1 flex flex-col h-full">
        {/* Toolbar */}
        <div
          className={`flex flex-wrap items-center gap-2 p-3 bg-muted/50 border-b ${compact ? "sticky top-0 z-10" : ""}`}
        >
          <Button
            variant="ghost"
            size="sm"
        ...
        {/* Editor */}
        <div
          className={`flex-1 p-4 bg-background focus:outline-none overflow-y-auto ${compact ? "min-h-[300px]" : "min-h-[600px]"}`}
        >
          <div
            ref={notesEditorRef}
            contentEditable
        ...
        (() => {
          ...
          const getThumbnailUrl = () => {
            if (hoveredBadge.type === "video") {
              return (
                (content as Video).thumbnail_url || (content as Video).gif_url
              );
            } else if (hoveredBadge.type === "photo") {
              return (
                fixSupabaseStorageUrl((content as PhotoCarousel).photo_url_1) ||
                ""
              );
            } else {
              return (
                (content as Reel).thumbnail_url ||
                (content as Reel).video_file_url ||
                ""
              );
            }
          };
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 text-xs text-slate-500">
                      <span>
                        👥{" "}
                        {formatNumber(Number(content.profile_followers) || 0)}{" "}
                        followers
                      </span>
                    </div>
                  </div>
