...
import { Music, Mic, Heart, Zap, Volume2, Waves, Sparkles, FileText, Quote, Eye, Film, Palette } from 'lucide-react';
...
export const VideoAnalysisCharts = ({ analysisData }: VideoAnalysisChartsProps) => {
  if (!analysisData) {
    return null;
  }
  ...
  // Get audio_analysis and video_analysis from the hook item
  const audioAnalysis = hookItem?.audio_analysis || {};
  const videoAnalysis = hookItem?.video_analysis || {};

  console.log('🎬 VideoAnalysisCharts received:', { 
    hasHooksData: !!hooksData, 
    hookItemType: hookItem ? typeof hookItem : 'none',
    audioAnalysisKeys: Object.keys(audioAnalysis),
    videoAnalysisKeys: Object.keys(videoAnalysis)
  });
  ...
  // Parse video analysis fields
  const contentStyleData = safeParse(videoAnalysis.content_style);
  const visualElementsData = safeParse(videoAnalysis.visual_elements);
  const hookTypeData = safeParse(videoAnalysis.hook_type);
  const editingStyleData = safeParse(videoAnalysis.editing_style);
  const cameraWorkData = safeParse(videoAnalysis.camera_work);
  const paceData = safeParse(videoAnalysis.pace);
  const colorPaletteData = safeParse(videoAnalysis.color_palette);
  ...
  // Convert content style object to array
  const contentStyleChartData = contentStyleData
    ? Object.entries(contentStyleData)
        .map(([style, value]) => ({
          style,
          value: typeof value === 'number' ? value * 100 : 0
        }))
  ...
  // Convert visual elements to array
  const visualElementsChartData = visualElementsData
    ? Object.entries(visualElementsData)
        .map(([element, value]) => ({
          element,
          value: typeof value === 'number' ? value * 100 : 0
        }))
  ...
  const hasAudioData = moodChartData.length > 0 || genreChartData.length > 0 || 
                  instrumentsChartData.length > 0 || emotionalProfile || 
                  voicesData || technicalFeedback || lyricAnalysis;

  const hasVideoData = contentStyleChartData.length > 0 || visualElementsChartData.length > 0 ||
                  hookTypeData || editingStyleData || cameraWorkData || paceData;
  ...
  return (
    <div className="mt-6 pt-6 border-t border-border/30 space-y-8">
      {/* Video/Visual Analysis Section */}
      {hasVideoData && (
        <div>
          <div className="flex items-center gap-2 mb-4">
            <Film className="w-4 h-4 text-sky-500" />
            <h3 className="text-sm font-semibold text-foreground">Visual Analysis</h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            
            {/* Content Style */}
            {contentStyleChartData.length > 0 && (
              <Section>
                <SectionHeader icon={Eye} title="Content Style" />
                <div className="space-y-2">
                  {contentStyleChartData.slice(0, 4).map((item, i) => (
                    <MiniProgress key={i} label={item.style} value={item.value} color="sky" />
                  ))}
                </div>
              </Section>
            )}
            ...
            {/* Visual Elements */}
            {visualElementsChartData.length > 0 && (
              <Section>
                <SectionHeader icon={Palette} title="Visual Elements" />
                <div className="space-y-2">
                  {visualElementsChartData.map((item, i) => (
                    <MiniProgress key={i} label={item.element} value={item.value} color="accent" />
                  ))}
                </div>
              </Section>
            )}
            ...
            {/* Video Details */}
            {(hookTypeData || editingStyleData || paceData || cameraWorkData) && (
              <Section>
                <SectionHeader icon={Film} title="Video Details" />
                <div className="flex flex-wrap gap-2">
                  {hookTypeData && (
                    <Badge variant="secondary" className="text-xs font-normal">
                      Hook: {typeof hookTypeData === 'string' ? hookTypeData : hookTypeData.type || 'N/A'}
                    </Badge>
                  )}
                  {editingStyleData && (
                    <Badge variant="outline" className="text-xs font-normal">
                      {typeof editingStyleData === 'string' ? editingStyleData : editingStyleData.style || 'N/A'}
                    </Badge>
                  )}
                  {paceData && (
                    <Badge variant="outline" className="text-xs font-normal">
                      {typeof paceData === 'string' ? paceData : paceData.level || 'N/A'} Pace
                    </Badge>
                  )}
                  {cameraWorkData && (
                    <Badge variant="outline" className="text-xs font-normal">
                      {typeof cameraWorkData === 'string' ? cameraWorkData : cameraWorkData.type || 'N/A'}
                    </Badge>
                  )}
                </div>
                {colorPaletteData && Array.isArray(colorPaletteData) && colorPaletteData.length > 0 && (
                  <div className="flex gap-1 mt-2">
                    {colorPaletteData.slice(0, 6).map((color: string, i: number) => (
                      <div 
                        key={i} 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            {technicalFeedback && (
              <Section>
                ...
                <div className="flex flex-wrap gap-2">
                  {technicalFeedback.tempo_bpm && (
                    <div className="flex items-center gap-1.5 px-2.5 py-1 bg-muted/30 rounded-full">
                      <span className="text-xs text-muted-foreground">BPM</span>
                      <span className="text-xs font-medium">{technicalFeedback.tempo_bpm}</span>
                    </div>
                  )}
