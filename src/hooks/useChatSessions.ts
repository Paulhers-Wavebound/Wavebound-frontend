...
export function useChatSessions() {
  ...
  const STORAGE_KEY = 'wavebound_chat_current_session_id';
  ...
  // In-memory message cache for instant session switching
  ...
  const loadSessions = async () => {
    ...
    if (!error && data) {
      setSessions(data.map(s => ({ ...s, is_favorite: (s as any).is_favorite ?? false })));
      if (currentSessionId) {
  ...
  // Shared fetch logic
  const fetchMessages = async (sessionId: string): Promise<ChatMessage[] | null> => {
    const { data, error } = await supabase
      .from('chat_messages')
      .select('*')
    ...
    if (error) {
      console.error('[useChatSessions.loadMessages] Failed to load chat_messages:', error);
      return null;
    }
    ...
    if (!hasAssistant) {
      ...
      if (n8nError) {
        console.warn('[useChatSessions.loadMessages] Failed to load n8n_chat_histories fallback:', n8nError);
        return baseMessages;
      }
  ...
  const loadSessionIdeas = async (sessionId: string) => {
    ...
    const needsEnrichment = ideas.filter((i) => {
      ...
      return !vd?.thumbnail_url && !vd?.video_url && !vd?.video_embedded_url && !vd?.post_url;
    });

    if (needsEnrichment.length > 0) {
      const numericIds = needsEnrichment
        .map((i) => parseInt(String(i.video_embed_id || i.id).replace(/^(video_|reel_|pc_)/, ''), 10))
        .filter((n) => !isNaN(n));

      const tiktokIds = numericIds.filter((id) => needsEnrichment.find((i) => (i.contentType === 'tiktok' || !i.contentType) && parseInt(String(i.video_embed_id || i.id).replace(/\D/g, ''), 10) === id));
      const reelIds = numericIds.filter((id) => needsEnrichment.find((i) => i.contentType === 'reel' && parseInt(String(i.video_embed_id || i.id).replace(/\D/g, ''), 10) === id));
      const pcIds = numericIds.filter((id) => needsEnrichment.find((i) => i.contentType === 'photo_carousel' && parseInt(String(i.video_embed_id || i.id).replace(/\D/g, ''), 10) === id));

      const [tiktokVideos, tiktokAssets, reels, reelAssets, reelMedia, pcs, pcAssets] = await Promise.all([
        tiktokIds.length ? supabase.from('0.1. Table 2 - Video - TikTok').select('id, video_embedded_url, video_url, caption, video_views, video_likes, duration, viral_score, performance_multiplier').in('id', tiktokIds) : { data: [] },
        tiktokIds.length ? supabase.from('0.1. Table 4 - Assets - TikTok').select('video_id, thumbnail_url').in('video_id', tiktokIds) : { data: [] },
        reelIds.length ? supabase.from('0.1. Table 2.2 - Video - Reels').select('id, video_embedded_url, video_url, caption, video_views, video_likes, duration, viral_score, performance_multiplier').in('id', reelIds) : { data: [] },
        reelIds.length ? supabase.from('0.1. Table 4.2 - Assets - Reels').select('video_id, thumbnail_url').in('video_id', reelIds) : { data: [] },
        reelIds.length ? supabase.from('media_assets_instagram_reels').select('video_id, url').in('video_id', reelIds) : { data: [] },
        pcIds.length ? supabase.from('0.1. Table 2.1 - PC - TikTok').select('id, post_url, caption, photo_views, photo_likes, viral_score, performance_multiplier').in('id', pcIds) : { data: [] },
        pcIds.length ? supabase.from('0.1. Table 4.1 - Assets - PC - TikTok').select('video_id, thumbnail_url').in('video_id', pcIds) : { data: [] },
      ]);
      ...
      ((tiktokAssets.data as any[]) || []).forEach((a) => a.video_id && a.thumbnail_url && tiktokThumb.set(a.video_id, a.thumbnail_url));

      const reelMap = new Map<number, any>();
      ((reels.data as any[]) || []).forEach((v) => reelMap.set(v.id, v));
      const reelThumb = new Map<number, string>();
      ((reelAssets.data as any[]) || []).forEach((a) => a.video_id && a.thumbnail_url && reelThumb.set(a.video_id, a.thumbnail_url));
      const reelFile = new Map<number, string>();
      ((reelMedia.data as any[]) || []).forEach((a) => a.video_id && a.url && reelFile.set(a.video_id, a.url));

      const pcMap = new Map<number, any>();
      ((pcs.data as any[]) || []).forEach((v) => pcMap.set(v.id, v));
      const pcThumb = new Map<number, string>();
      ((pcAssets.data as any[]) || []).forEach((a) => a.video_id && a.thumbnail_url && pcThumb.set(a.video_id, a.thumbnail_url));

      for (const idea of ideas) {
        const idNum = parseInt(String(idea.video_embed_id || idea.id).replace(/\D/g, ''), 10);
  ...
  const createSession = useCallback(async (title?: string): Promise<string | null> => {
    ...
    messageCacheRef.current.set(newId, []);

    // Background persist
    supabase
      .from('chat_sessions')
      .insert({ id: newId, user_id: userId, title: title || 'New Chat' })
      .select()
      .single()
      .then(({ data, error }) => {
        if (error) {
          console.error('[useChatSessions] Failed to persist session, rolling back:', error);
          setSessions(prev => prev.filter(s => s.id !== newId));
          messageCacheRef.current.delete(newId);
        } else if (data) {
          // Update with real server data
          setSessions(prev => prev.map(s =>
            s.id === newId ? { ...data, is_favorite: (data as any).is_favorite ?? false } : s
          ));
        }
      });
  ...
  const persistDraftSession = useCallback(async (sessionId: string, title?: string): Promise<boolean> => {
    if (!userId || !isDraftSession) return true;
    
    const { data, error } = await supabase
      .from('chat_sessions')
      .insert({ id: sessionId, user_id: userId, title: title || 'New Chat' })
      .select()
      .single();

    if (!error && data) {
      setSessions(prev => [{ ...data, is_favorite: (data as any).is_favorite ?? false }, ...prev]);
      setIsDraftSession(false);
      return true;
    }
    console.error('[useChatSessions] Failed to persist draft session:', error);
    return false;
  ...
  const toggleFavorite = useCallback(async (sessionId: string) => {
    ...
    setSessions(prev => {
      ...
      return prev.map(s => s.id === sessionId ? { ...s, is_favorite: newValue } : s);
    });
    // Background persist
    const session = sessions.find(s => s.id === sessionId);
    if (session) {
      supabase.from('chat_sessions').update({ is_favorite: !session.is_favorite } as any).eq('id', sessionId);
    }
  }, [sessions]);
  ...
  const addMessage = useCallback(async (
    ...
    if (!error && data) {
      ...
      messageCacheRef.current.set(sessionId, [
        ...(messageCacheRef.current.get(sessionId) || []),
        message,
      ]);
      ...
      if (role === 'user') {
        setSessions(prev => {
          ...
          const title = needsTitle ? generateChatTitle(content) : session.title;
          const updated = prev.map(s =>
            s.id === sessionId ? { ...s, title, updated_at: newTimestamp } : s
          );
          // Background DB persist
          const updatePayload = needsTitle
            ? { title, updated_at: newTimestamp }
            : { updated_at: newTimestamp };
          supabase.from('chat_sessions').update(updatePayload).eq('id', sessionId);
    ...
    console.error('[useChatSessions.addMessage] Failed to save message:', error);
    return null;
  }, []);

  const addIdea = useCallback(async (sessionId: string, idea: EnrichedIdea): Promise<boolean> => {
    ...
    if (error) {
      console.error('[useChatSessions.addIdea] Failed to insert idea', { sessionId, ideaId: idea.id, error });
      return false;
    }
  ...
  const removeIdea = useCallback(async (sessionId: string, ideaId: string): Promise<boolean> => {
    const { error } = await supabase
      .from('session_ideas')
      .delete()
      .eq('session_id', sessionId)
  ...
  const clearSessionIdeas = useCallback(async (sessionId: string): Promise<boolean> => {
    const { error } = await supabase
      .from('session_ideas')
      .delete()
      .eq('session_id', sessionId);
