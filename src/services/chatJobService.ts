...
/**
 * Stream a chat message using SSE from the artist-chat edge function.
...
export async function streamChatMessage(
  ...
  const response = await fetch(`${SUPABASE_URL}/functions/v1/artist-chat`, {
    method: 'POST',
    headers: {
            ...
            try {
              ...
              switch (eventType) {
                ...
                default:
                  ...
                  if (parsed.status) callbacks.onStatus?.(null as any, parsed.status);
                  break;
              }
            } catch {}
          }
