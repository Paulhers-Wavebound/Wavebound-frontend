...
export default function LabelRosterPage() {
  ...
  const sorted = [...enriched].sort((a, b) => {
    switch (sortBy) {
      ...
      case "frequency":
        return (a.freq_tt || 999) - (b.freq_tt || 999);
      case "attention": {
        const score = (x: typeof a) => {
          if (!x.last_post_date) return 0;
          const days =
            (Date.now() - new Date(x.last_post_date).getTime()) / 86400000;
  ...
  if (roleLoading || (loading && !error)) {
    return (
      <>
        <SEOHead
          title="Roster — Wavebound Label"
          description="Your artist roster"
        />
          ...
          {[0, 1, 2, 3, 4].map((i) => (
            <div
              ...
              style={{
                ...
                animation: `labelPulse 1.5s infinite ${i * 100}ms`,
              }}
            >
              <div
                style={{
          ...
          >
            {FREQ_OPTIONS.map((opt) => (
              <button
                ...
                style={{
                  background:
                    freqWindow === opt.value ? "var(--accent)" : "transparent",
                  color:
                    freqWindow === opt.value ? "#fff" : "var(--ink-tertiary)",
                }}
              >
                {opt.label}
              </button>
      ...
      <div style={{ marginTop: 24 }}>
        {sorted.length === 0 ? (
          ...
          >
            ...
            >
              Artists will appear here once they've been analyzed.
            </div>
          </div>
        ) : (
          sorted.map((artist, i) => (
