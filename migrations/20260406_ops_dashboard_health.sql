-- Ops Dashboard — 6 new sections for get_admin_health_extended()
-- Adds: sc_credit_history, yt_quota_history, platform_id_trend, platform_id_daily,
--        unresolved_artists, scraper_run_history
-- Applied: 2026-04-06
...
BEGIN
  ...
  -- ═══════════════════════════════════════════════════
  -- MATERIALIZE: scan wb_observations once, reuse everywhere
  -- ═══════════════════════════════════════════════════
  ...
  SELECT jsonb_build_object(
    'obs_freshness', COALESCE((
      SELECT jsonb_agg(
        jsonb_build_object(
          'platform', t.platform,
          'last_observation_at', t.last_obs,
          'observation_count', t.total_obs
        ) ORDER BY t.last_obs DESC
      )
      FROM _health_plat_stats t
    ), '[]'::jsonb),
    ...
    'geo_freshness', COALESCE((
      SELECT jsonb_agg(
        jsonb_build_object(
          'platform', t.platform,
          'last_observation_at', t.last_obs,
          'observation_count', t.cnt,
          'unique_countries', t.countries
        ) ORDER BY t.last_obs DESC
      )
      FROM (
  ...
  SELECT jsonb_build_object(
    'first_observation_date', (
      SELECT MIN(last_obs)::date FROM _health_plat_stats
    )::text
  ) INTO v_section;
  v_result := v_result || v_section;
  ...
  SELECT jsonb_build_object(
    ...
    'artists_with_multi_platform', (
      SELECT COUNT(*)::bigint FROM (
        ...
        JOIN wb_entities e ON e.id = ep.entity_id
        WHERE e.entity_type = 'artist'
  ...
  SELECT jsonb_build_object(
    'platform_breakdown', COALESCE((
      SELECT jsonb_agg(
        jsonb_build_object(
          ...
          'total_observations', ps.total_obs,
          'unique_entities', ps.uniq_ent,
        ...
        ) ORDER BY ps.total_obs DESC
      )
      FROM _health_plat_stats ps
    ), '[]'::jsonb),
    ...
    'platform_geo_breakdown', COALESCE((
      SELECT jsonb_agg(
        jsonb_build_object(
          'platform', t.platform,
          'total_geo_observations', t.total_geo,
          'unique_countries', t.countries
        ) ORDER BY t.total_geo DESC
      )
      FROM (
        SELECT platform,
               COUNT(*)::bigint as total_geo,
               COUNT(DISTINCT country_code)::int as countries
        FROM wb_observations_geo GROUP BY platform
      ) t
    ), '[]'::jsonb)
  ...
  SELECT jsonb_build_object(
    ...
    'top_countries', COALESCE((
      ...
      FROM (
        ...
        FROM wb_observations_geo og
        JOIN wb_entities e ON e.id = og.entity_id AND e.entity_type = 'artist'
    ...
    'top_platforms', COALESCE((
      ...
      FROM (
        ...
        FROM _health_ep ep
        JOIN wb_entities e ON e.id = ep.entity_id AND e.entity_type = 'artist'
        GROUP BY e.id, e.canonical_name ORDER BY cnt DESC LIMIT 5
      ) t
    ), '[]'::jsonb),
    ...
    'newest_entities', COALESCE((
      ...
      FROM (
        ...
        FROM wb_entities ORDER BY created_at DESC LIMIT 10
      ) t
    ), '[]'::jsonb)
  ) INTO v_section;
  v_result := v_result || v_section;
  ...
  SELECT jsonb_build_object(
    ...
    'zero_heavy_metrics', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'platform', t.platform, 'metric', t.metric, 'zero_pct', t.zpct
      ))
      FROM (
  ...
  -- ═══════════════════════════════════════════════════
  -- OPS: SC CREDIT HISTORY + YOUTUBE QUOTA
  -- ═══════════════════════════════════════════════════
  SELECT jsonb_build_object(
  ...
  -- ═══════════════════════════════════════════════════
  -- OPS: PLATFORM ID COVERAGE TREND
  -- ═══════════════════════════════════════════════════
  SELECT jsonb_build_object(
  ...
  -- ═══════════════════════════════════════════════════
  -- OPS: UNRESOLVED ARTISTS (0 platform_ids)
  -- ═══════════════════════════════════════════════════
  SELECT jsonb_build_object(
    'unresolved_artists', (
  ...
  -- ═══════════════════════════════════════════════════
  -- OPS: SCRAPER RUN HISTORY (48h, cron gap detection)
  -- ═══════════════════════════════════════════════════
  SELECT jsonb_build_object(
