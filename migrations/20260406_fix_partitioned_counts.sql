-- Fix: PostgREST returns 0 for partitioned table counts (wb_observations, wb_observations_geo)
-- Solution: add total_observations and total_observations_geo to the RPC output
-- These are derived from the already-materialized _health_plat_stats temp table (no extra scan)
...
BEGIN
  ...
  -- ═══ PARTITIONED TABLE COUNTS (PostgREST can't count these) ═══
  ...
  SELECT jsonb_build_object(
    'platform_breakdown', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'platform', ps.platform, 'total_observations', ps.total_obs,
